...
# - 존재하지 않으면 에러 응답
@app.get("/select_product_image/{item_id}")
...
async def insert_one( productImage : ProductImage
    # TODO: Form 파라미터 정의
    ...
    try:
        ...
        # TODO: SQL 작성
        sql = """
            INSERT INTO ProductImage (pbid, imagePath) 
            VALUES (%s, %s)
        """
        curs.execute(sql, (productImage.pbid, productImage.imagePath))
...
async def update_one(
    ...
    try:
        ...
        sql = """
            UPDATE ProductImage
            SET pbid=%s, imagePath=%s
            WHERE id=%s
        """
        curs.execute(sql, (pbid, imagePath, id))
...
@app.delete("/delete_product_image/{item_id}")
...
if __name__ == "__main__":
    import uvicorn
    print(f"🚀 product_image API 서버 시작")
